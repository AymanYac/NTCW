...
EXECUTE format('
   CREATE TABLE IF NOT EXISTS %I.project_rules(rule_id character varying primary key, rule_name character varying,main character varying,application character varying,complement character varying,material_group character varying,pre_classification character varying,drawing boolean,class_id character varying,rule_source character varying,rule_type character varying,rule_level int,rule_scoring double precision,rule_rank int,rule_accuracy double precision,rule_baseline int,user_id character varying,rule_date date, active_status boolean, source_project_id character varying)',  project_id);   
...
EXECUTE format('
   CREATE TABLE IF NOT EXISTS %I.project_description_patterns(generic_char_rule_id character varying, source_characteristic_id character varying, generic_char_rule_json character varying, isSuppressed boolean, PRIMARY KEY (generic_char_rule_id)
)',  project_id);
...
EXECUTE format('
   CREATE TABLE IF NOT EXISTS %I.project_characteristics_x_values(characteristic_id character varying ,user_id character varying, description_method character varying, description_time timestamp with time zone,value_id character varying, description_rule_id character varying, url_link character varying
)',  project_id);
