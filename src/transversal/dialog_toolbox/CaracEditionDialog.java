...
public class CaracEditionDialog {
			...
			public void handle(ActionEvent event) {
				if(resultTable.getSelectionModel().getSelectedIndices().size()>1){
					resultTable.getSelectionModel().getSelectedItems().forEach(r->{
						...
						dialog.close();
						CaracEditionDialog.dialog.close();
						return;
					});
				}
	...
	public static void CaracEditionPopUp(ClassCaracteristic carac, UserAccount account, TableView<CharDescriptionRow> tableGrid,Char_description parent) throws SQLException, ClassNotFoundException {
		...
		CaracEditionDialog.CaracDeclarationPopUp(account, itemSegement,entry,parent,null);
	...
	public static void CaracDeletion(ClassCaracteristic carac, String itemSegment,UserAccount account) throws SQLException, ClassNotFoundException {
		...
		if(scopeChoice!=null){
			...
			if(impactedValuesOnDelete>0){
				if(!ConfirmationDialog.showCaracOrValueDeleteImpactConfirmation(String.valueOf(impactedValuesOnDelete)+" value(s) attached to this characteristic will be lost!")){
					return;
				}
			}
    ...
    public static void CaracValueListEdit(ClassCaracteristic carac, String segmentID, Char_description parent) {
		...
		CustomTableColumn<String,String> valueColumn = new CustomTableColumn<>();
		valueColumn.setPercentageWidth(0.73);
		...
		CustomTableColumn<String,Button> dropColumn = new CustomTableColumn<String,Button>();
		dropColumn.setPercentageWidth(0.1);
		dropColumn.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<String, Button>, ObservableValue<Button>>() {
			@Override
			public ObservableValue<Button> call(TableColumn.CellDataFeatures<String, Button> param) {
				Button tmp = new Button();
				tmp.setId("dropButton");
				tmp.setOnAction(action->{
					int impactedValuesOnDelete = (int) CharItemFetcher.allRowItems.stream().filter(r->(r.getClass_segment_string().split("&&&")[0].equals(segmentID)))
							.filter(r->r.hasDataInCurrentClassForCarac(carac.getCharacteristic_id())).filter(r->r.getData(segmentID).get(carac.getCharacteristic_id()).getDataLanguageValue().equals(param.getValue())).count();
					if(impactedValuesOnDelete>0){
						if(!ConfirmationDialog.showCaracOrValueDeleteImpactConfirmation(String.valueOf(impactedValuesOnDelete)+" value(s) attached to this characteristic will be lost!")){
							return;
						}else{
							CharItemFetcher.allRowItems.stream().filter(r->(r.getClass_segment_string().split("&&&")[0].equals(segmentID)))
									.filter(r->r.hasDataInCurrentClassForCarac(carac.getCharacteristic_id())).filter(r->r.getData(segmentID).get(carac.getCharacteristic_id()).getDataLanguageValue().equals(param.getValue()))
									.forEach(r->{
										r.getData(segmentID).remove(carac.getCharacteristic_id());
										parent.refresh_ui_display();
					...
					tableView.getItems().remove(param.getValue());
				});
				return new ReadOnlyObjectWrapper<>(tmp);
			}
		});
		CustomTableColumn<String,Button> editColumn = new CustomTableColumn<String,Button>();
		editColumn.setPercentageWidth(0.1);
		editColumn.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<String, Button>, ObservableValue<Button>>() {
			@Override
			public ObservableValue<Button> call(TableColumn.CellDataFeatures<String, Button> param) {
				Button tmp = new Button();
				tmp.setOnAction(action->{
					String ret = ConfirmationDialog.showCaracValueEdition(dialog.getTitle(), param.getValue());
					if(ret!=null){
						int impactedValuesOnDelete = (int) CharItemFetcher.allRowItems.stream().filter(r->(r.getClass_segment_string().split("&&&")[0].equals(segmentID)))
								.filter(r->r.hasDataInCurrentClassForCarac(carac.getCharacteristic_id())).filter(r->r.getData(segmentID).get(carac.getCharacteristic_id()).getDataLanguageValue().equals(param.getValue())).count();
						if(impactedValuesOnDelete>0){
							if(!ConfirmationDialog.showCaracOrValueDeleteImpactConfirmation(String.valueOf(impactedValuesOnDelete)+" value(s) attached to this characteristic will be edited!")){
								return;
							}else{
								CharItemFetcher.allRowItems.stream().filter(r->(r.getClass_segment_string().split("&&&")[0].equals(segmentID)))
										.filter(r->r.hasDataInCurrentClassForCarac(carac.getCharacteristic_id())).filter(r->r.getData(segmentID).get(carac.getCharacteristic_id()).getDataLanguageValue().equals(param.getValue()))
										.forEach(r->{
											...
											val.setDescriptionTime(Timestamp.valueOf(LocalDateTime.now()));
											val.setManually_Reviewed(true);
											val.setSource(DataInputMethods.MANUAL);
											val.setDataLanguageValue(ret);
											val.setUserLanguageValue(TranslationServices.getEntryTranslation(ret,true));
											r.getData(segmentID).put(carac.getCharacteristic_id(),val);
											parent.refresh_ui_display();
											parent.tableController.charDescriptionTable.refresh();
											CharDescriptionExportServices.addItemCharDataToPush(r);
										});
							...
							tableView.getItems().remove(param.getValue());
							tableView.getItems().remove(ret);
							tableView.getItems().add(ret);
							tableView.getItems().sort(Comparator.naturalOrder());

						}
					}
				});
				tmp.setId("editButton");
				return new ReadOnlyObjectWrapper<>(tmp);
			}
		});

		tableView.getColumns().addAll(valueColumn,dropColumn,editColumn);
		tableView.getItems().addAll(TranslationServices.getTextEntriesForCharOnLanguages(carac,true).stream().map(CharValueTextSuggestion::getSource_value)
				.filter(str->str.trim().length()>0).sorted().collect(Collectors.toCollection(ArrayList::new)));
		grid.add(tableView,0,0);
